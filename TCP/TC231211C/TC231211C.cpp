...
static std::string demangle_func_name(const std::string& mangled_name) {
  int status;
...
static bool is_cxx_class_func(const std::string& func_name){
  ...
  bool runOnFunction(Function &F) override {
    ...
    for (auto& BB : F) {
      for(BasicBlock::iterator i = BB.begin(); i != BB.end(); ++i){
        ...
        if( !is_cxx_class_func(func_name) )
            continue;
